...
void perf_event_print_debug(void)
{
	...
	if (!ppmu) {
		pr_debug("Performance monitor hardware not registered.\n");
		return;
	}
	...
	pr_debug("CPU: %d PMU registers, ppmu = %s n_counters = %d",
	...
	pr_debug("PMC1:  %08x PMC2: %08x PMC3: %08x PMC4: %08x\n",
		 pmcs[0], pmcs[1], pmcs[2], pmcs[3]);

	if (ppmu->n_counter > 4)
		pr_debug("PMC5:  %08x PMC6: %08x PMC7: %08x PMC8: %08x\n",
			 pmcs[4], pmcs[5], pmcs[6], pmcs[7]);

	pr_debug("MMCR0: %016lx MMCR1: %016lx MMCRA: %016lx\n",
	...
	if (ppmu->flags & PPMU_ARCH_207S) {
		pr_debug("MMCR2: %016lx EBBHR: %016lx\n",
			mfspr(SPRN_MMCR2), mfspr(SPRN_EBBHR));
		pr_debug("EBBRR: %016lx BESCR: %016lx\n",
			mfspr(SPRN_EBBRR), mfspr(SPRN_BESCR));
	}
#endif
	pr_debug("SIAR:  %016lx SDAR:  %016lx SIER:  %016lx\n",
