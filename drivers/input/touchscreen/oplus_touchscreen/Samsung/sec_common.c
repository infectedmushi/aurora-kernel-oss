...
static ssize_t sec_ts_regread_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    do {
        ...
        if (ret < 0) {
            TPD_DEBUG("%s: i2c read %x command, remain =%d\n", __func__, lv1cmd, remain);
            goto i2c_err;
        }
...
void sec_raw_device_init(struct touchpanel_data *ts)
{
    ...
    if (ret) {
        TPD_DEBUG("%s: fail to create class\n", __func__);
        return;
    }
    ...
    ret = IS_ERR(sec_dev);
    if (ret) {
        TPD_DEBUG("%s: fail - device_create\n", __func__);
        return;
    }

    ret = sysfs_create_group(&sec_dev->kobj, &cmd_attr_group);
    if (ret < 0) {
        TPD_DEBUG("%s: fail - sysfs_create_group\n", __func__);
        goto err_sysfs;
    }

    TPD_DEBUG("create debug interface success\n");
    return;
err_sysfs:
    TPD_DEBUG("%s: fail\n", __func__);
    return;
