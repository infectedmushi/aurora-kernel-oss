...
static int sec_enable_black_gesture(struct chip_data_s6sy771 *chip_info, bool enable)
{
	...
	TPD_DEBUG("%s, enable = %d,single_tap_support_a = %d,ear_sense_support_a =%d\n", __func__, enable, single_tap_support_a, ear_sense_support_a);
    if (enable) {
    ...
    if (i >= 20) {
        ret = -1;
        TPD_DEBUG("%s: %s black gesture failed\n", __func__, enable ? "enter" : "exit");
    } else {
        TPD_DEBUG("%s: %d times %s black gesture success\n", __func__, i, enable ? "enter" : "exit");
    }
    return ret;
}
...
static void sec_enable_gesture_mask(void *chip_data, uint32_t enable)
{
	...
	if (ts) {
        ...
        TPD_DEBUG("%s, enable = %d,single_tap_support_a =%d\n", __func__, enable, single_tap_support_a);
...
void sec_refresh_switch(int fps)
{
	...
	if(g_chip_info->sec_refresh_rate_delay) {
	    ...
	    if (!g_chip_info->is_power_down) {
			...
			TPD_DEBUG("%s: refresh_switch: %d HZ %s! 100ms\n", __func__, fps, ret < 0 ? "failed" : "success");
		...
		if (!g_chip_info->is_power_down) {
			ret = touch_i2c_write_byte(g_chip_info->client, SEC_CMD_REFRESH_RATE_SWITCH, fps);
			TPD_DEBUG("%s: refresh_switch: %d HZ %s!\n", __func__, fps, ret < 0 ? "failed" : "success");
		}
	}
    return;
}
...
static int sec_get_chip_info(void *chip_data)
{
    ...
    if (ret < 0) {
        TPD_DEBUG("%s: failed to read default lock point level\n", __func__);
    } else {
        chip_info->default_lock_point_level = buf[3];
        TPD_DEBUG("%s: get default lock point level : 0x%2x\n", __func__, chip_info->default_lock_point_level);
    }
...
static u32 sec_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
    ...
    if (event_id == SEC_STATUS_EVENT) {
        ...
        if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) && ((p_event_status->status_id == SEC_STATUS_PAGE_ERASE) ||
            ...
            TPD_DEBUG("%s: Flash change detected. 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n", __func__,
                chip_info->first_event[0], chip_info->first_event[1], chip_info->first_event[2], chip_info->first_event[3],
                chip_info->first_event[4], chip_info->first_event[5], chip_info->first_event[6], chip_info->first_event[7]);
            return IRQ_IGNORE;
        }
...
static int sec_mode_switch(void *chip_data, work_mode mode, bool flag)
{
	...
	case MODE_WIRELESS_CHARGE:
	    ...
	    if (ret < 0) {
	        TPD_DEBUG("%s: enable wireless charge mode : %d failed\n", __func__, flag);
	    }
	    break;
...
static void sec_earsese_delta_read(void *chip_data, char *earsense_delta, int read_len)
{
    ...
    if (!earsense_delta) {
        TPD_DEBUG("earsense_delta is NULL\n");
        return;
    }
...
static void sec_earsese_selfdata_read( void *chip_data, char *self_data, int read_len)
{
    ...
    if (ret < 0) {
        TPD_DEBUG("read selfdata failed\n");
        return;
    }
