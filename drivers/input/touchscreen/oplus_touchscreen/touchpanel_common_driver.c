...
void operate_mode_switch(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->mode_switch) {
		TPD_DEBUG("not support ts_ops->mode_switch callback\n");
		return;
	}
...
#endif
	{
		TPD_DEBUG("Ftm mode, no need to notify pen state\n");
		return;
	}
...
{
	if (!ts->ts_ops->reset) {
		TPD_DEBUG("not support ts->ts_ops->reset callback\n");
		return;
	}
...
static void tp_gesture_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->get_gesture_info) {
		TPD_DEBUG("not support ts->ts_ops->get_gesture_info callback\n");
		return;
	}
...
static void tp_touch_release(struct touchpanel_data *ts)
{
	...
	input_sync(ts->input_dev);
#endif
    TPD_DEBUG("release all touch point and key, clear tp touch down flag\n");
...
static void tp_touch_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->get_touch_points) {
		TPD_DEBUG("not support ts->ts_ops->get_touch_points callback\n");
		return;
	}
...
static void tp_btnkey_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->get_keycode) {
		TPD_DEBUG("not support ts->ts_ops->get_keycode callback\n");
...
static void tp_config_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->fw_handle) {
		TPD_DEBUG("not support ts->ts_ops->fw_handle callback\n");
		return;
	}
...
static void health_monitor_handle(struct touchpanel_data *ts)
{
	if (!ts->ts_ops->health_report) {
		TPD_DEBUG("not support ts->debug_info_ops->health_report callback\n");
		return;
	}
...
static void tp_face_detect_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->get_face_state) {
		TPD_DEBUG("not support ts->ts_ops->get_face_state callback\n");
		return;
	}
...
static void tp_fingerprint_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->screenon_fingerprint_info) {
		TPD_DEBUG("not support screenon_fingerprint_info callback.\n");
		return;
	}
...
static void tp_pen_handle(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->get_pen_points) {
		TPD_DEBUG("not support get_pen_points callback.\n");
		return;
	}
...
static void tp_async_work_callback(void)
{
...
#endif
	{
		TPD_DEBUG("%s: in ftm mode, no need to call back\n", __func__);
		return;
	}

	TPD_DEBUG("%s: async work\n", __func__);
	...
	if (ts->in_test_process) {
		TPD_DEBUG("%s: In test process, do not switch mode\n", __func__);
		return;
	}
...
static void tp_work_func(struct touchpanel_data *ts)
{
	...
	if (!ts->ts_ops->trigger_reason && !ts->ts_ops->u32_trigger_reason) {
		TPD_DEBUG("not support ts_ops->trigger_reason callback\n");
		return;
	}
	/*
...
static void tp_fw_update_work(struct work_struct *work)
{
	...
	if (!ts->ts_ops->fw_check || !ts->ts_ops->reset) {
		TPD_DEBUG("not support ts_ops->fw_check callback\n");
		complete(&ts->fw_complete);
		return;
	}

	TPD_DEBUG("%s: fw_name = %s\n", __func__, ts->panel_data.fw_name);
	...
	if (ts->ts_ops->fw_update) {
		...
		if (!ret || ts->is_noflash_ic) {
			...
			TPD_DEBUG("%s: fw_name request failed %s %d\n", __func__, ts->panel_data.fw_name, ret);
			if (ts->health_monitor_v2_support) {
...
static irqreturn_t tp_irq_thread_fn(int irq, void *dev_id)
{
	...
	if (ts->irq_need_dev_resume_ok) {
		if (ts->i2c_ready == false) {
			//TPD_DEBUG("Wait device resume!");
			wait_event_interruptible_timeout(waiter,
							 ts->i2c_ready,
							 msecs_to_jiffies(ts->msecs_to_jiffies_time));
			//TPD_DEBUG("Device maybe resume!");
		}
		if (ts->i2c_ready == false) {
			TPD_DEBUG("The device not resume %dms!", ts->msecs_to_jiffies_time);
			if (ts->ws) {
				__pm_relax(ts->ws);
			}
			return IRQ_HANDLED;
...
static void tp_freq_hop_work(struct work_struct *work)
{
	...
	TPD_DEBUG("syna_tcm_freq_hop_work\n");
	if (!ts->is_suspended) {
		TPD_DEBUG("trigger frequency hopping~~~~\n");

		if (!ts->ts_ops->freq_hop_trigger) {
			TPD_DEBUG("%s:not support ts_ops->freq_hop_trigger callback\n", __func__);
			return;
		}
	...
	if (ts->freq_hop_info.freq_hop_simulating) {
		TPD_DEBUG("queue_delayed_work again\n");
...
int tp_gesture_enable_flag(void)
{
	...
	TPD_DEBUG("g_tp->gesture_enable is %d\n", g_tp->gesture_enable);
...
int check_touchirq_triggered(void)
{
	...
	if ((0 == value) && (g_tp->irq_flags & IRQF_TRIGGER_LOW)) {
		TPD_DEBUG("touch irq is triggered.\n");
...
void switch_headset_state(int headset_state)
{
	...
	if (g_tp->headset_pump_support && (g_tp->is_headset_checked != headset_state)) {
		...
		TPD_DEBUG("%s: check headset state : %d, is_suspended: %d\n", __func__, headset_state, g_tp->is_suspended);
...
static ssize_t proc_gesture_control_indep_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (ts->sportify_aod_gesture_support && ts->is_suspended) {
		TPD_DEBUG("%s: now is suspend and sportify aod enable, change gesture mode \n", __func__);
		ts->ts_ops->mode_switch(ts->chip_data, MODE_GESTURE, true);
	}
...
static int gesture_support_fops_read_func(struct seq_file *s, void *v)
{
	...
	if (!ts) {
		TPD_DEBUG("%s : ts is NULL", __func__);
		goto OUT;
	}

	if (ts->sportify_aod_gesture_support) {
		ret = BIT(SingleTap);
		TPD_DEBUG("%s: sporitify aod support : %d", __func__, ret);
		seq_printf(s, "%d,\n", ret);
	} else {
		TPD_DEBUG("%s: not supprot special gesture type : %d", __func__, ret);
		seq_printf(s, "%d,\n", ret);
	}
OUT:
	return 0;
			...
			unsigned int array_max)
{
	...
	for (i = 0; i < in_cnt; i++) {
		if (':' == in[i]) {     /*split name and parameter by ":" symbol*/
			if (i > max_len) {
				TPD_DEBUG("string %s name too long.\n", in);
				return -1;
			}

			name_index = i;
			memcpy(name, in, name_index);   /*copy to name buffer*/
			TPD_DEBUG("set name %s.", name);
		}
	}
		...
		if (in[i] < '0' || in[i] > '9') {
			if ((' ' == in[i]) || (0 == in[i]) || ('\n' == in[i]) || (',' == in[i])) {
				if (value_cnt <= array_max) {
					...
					TPD_DEBUG("too many parameter(%s).", in);
					return -1;
				}
			} else {
				TPD_DEBUG("incorrect char 0x%02x in %s.", in[i], in);
				return -1;
			}
		}
	}
	...
	TPD_DEBUG("input para count is %d.", value_cnt);
	return value_cnt;
}
...
static ssize_t proc_game_para_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(buf, buffer, count)) {
		TPD_DEBUG("read proc input error.");
		return count;
	}

	if (value_cnt < 0) {
		TPD_DEBUG("str parse failed.");
		return count;
	}
	...
	if (strstr(name, "smooth_level") && (value_cnt == SMOOTH_LEVEL_NUM) && (ts->smooth_level_array_support)) {
		...
		TPD_DEBUG("smooth_level success");
	}
	...
	if (strstr(name, "smooth_charging_level") && (value_cnt == SMOOTH_LEVEL_NUM) && (ts->smooth_level_charging_array_support)) {
		...
		TPD_DEBUG("smooth_charging_level success");
	}
	...
	if (strstr(name, "sensitive_level") && (value_cnt == SENSITIVE_LEVEL_NUM) && (ts->sensitive_level_array_support)) {
		...
		TPD_DEBUG("sensitive_level success");
	}
...
static ssize_t proc_ps_status_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->write_ps_status) {
		TPD_DEBUG("not support ts_ops->write_ps_status callback\n");
		return count;
	}
				      ...
				      const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (count > 100) {
		TPD_DEBUG("%s:count > 100\n", __func__);
		return count;
	}

	if (!ts) {
		TPD_DEBUG("%s: ts is NULL\n", __func__);
		return count;
	}

	if (!ts->ts_ops->mode_switch) {
		TPD_DEBUG("%s:not support ts_ops->mode_switch callback\n", __func__);
		return count;
	}
	...
	TPD_DEBUG("%s: game_switch value=0x%x\n", __func__, value);
	...
	if (!ts->is_suspended) {
        ...
        TPD_DEBUG("%s: game_switch_support is_suspended.\n", __func__);
    }
...
static ssize_t proc_black_screen_test_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (ts->ts_ops->black_screen_test) {
		...
		TPD_DEBUG("black_screen_test not support\n");
...
static ssize_t proc_limit_area_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	TPD_DEBUG("limit_area = %d; left_x1 = %d; right_x1 = %d; left_x2 = %d; right_x2 = %d; left_x3 = %d; right_x3 = %d\n",
		 ts->edge_limit.limit_area, ts->edge_limit.left_x1, ts->edge_limit.right_x1, ts->edge_limit.left_x2, ts->edge_limit.right_x2, ts->edge_limit.left_x3, ts->edge_limit.right_x3);
	...
	TPD_DEBUG("limit_area = %d; left_y1 = %d; right_y1 = %d; left_y2 = %d; right_y2 = %d; left_y3 = %d; right_y3 = %d\n",
...
static ssize_t proc_limit_control_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	TPD_DEBUG("limit_enable is: 0x%x, ts->limit_edge = 0x%x, ts->limit_corner = 0x%x\n", ts->limit_enable, ts->limit_edge, ts->limit_corner);
	ret = snprintf(page, PAGESIZE - 1, "%d\n", ts->limit_enable);
...
static ssize_t proc_limit_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	TPD_DEBUG("%s: limit_enable = 0x%x, ts->limit_edge = 0x%x, ts->limit_corner=0x%x\n", __func__, ts->limit_enable, ts->limit_edge, ts->limit_corner);
...
static ssize_t proc_fw_update_write(struct file *file, const char __user *page, size_t size, loff_t *lo)
{
...
#endif
	{
		TPD_DEBUG("boot mode is MSM_BOOT_MODE__CHARGE,not need update tp firmware\n");
		return size;
	}
	...
	if (ret < 0) {
		TPD_DEBUG("kill signal interrupt\n");
	}

#ifdef CONFIG_TOUCHIRQ_UPDATE_QOS
	if (!pm_qos_state) {
		pm_qos_add_request(&pm_qos_req, PM_QOS_CPU_DMA_LATENCY, pm_qos_value);
		TPD_DEBUG("add qos request in touch driver.\n");
		pm_qos_state = 1;
	}
#endif

	TPD_DEBUG("fw update finished\n");
	return size;
}
...
static ssize_t proc_aging_test_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (!ret) {
		TPD_DEBUG("%s:no support aging_test2\n", __func__);
	}
...
static int tp_test_limit_switch(struct touchpanel_data *ts)
{
	...
	if (ts->panel_data.aging_test_limit_name == NULL) {
		TPD_DEBUG("[TP]panel_data.test_limit_name kzalloc error\n");
		return -1;
	}
	...
	TPD_DEBUG("aging_test_limit_name is %s\n", ts->panel_data.aging_test_limit_name);
	return 0;
...
static int finger_protect_handler(void *data)
{
	...
	if (!ts) {
		TPD_DEBUG("ts is null should nerver get here!\n");
		return 0;
	};
	if (!ts->ts_ops->spurious_fp_check) {
		TPD_DEBUG("not support spurious_fp_check call back\n");
		return 0;
	}
	...
	do {
		...
		if (g_tp->spuri_fp_touch.lcd_trigger_fp_check && !g_tp->spuri_fp_touch.lcd_resume_ok) {
			TPD_DEBUG("LCD is suspend, can not detect finger touch in incell panel\n");
			mutex_unlock(&ts->mutex);
			continue;
		}
		...
		if (ts->view_area_touched) {
			TPD_DEBUG("%s tp touch down,clear flag\n", __func__);
			ts->view_area_touched = 0;
		}
		operate_mode_switch(ts);
		mutex_unlock(&ts->mutex);
...
static ssize_t proc_register_info_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (count > 7) {
		TPD_DEBUG("%s count = %ld\n", __func__, count);
		return count;
	}
	if (!ts) {
		TPD_DEBUG("ts not exist!\n");
		return count;
	}
	if (copy_from_user(buf, buffer, count)) {
		TPD_DEBUG("%s: read proc input error.\n", __func__);
	...
	TPD_DEBUG("ts->reg_info.reg_addr = 0x%x, ts->reg_info.reg_lenght = %d\n", ts->reg_info.reg_addr, ts->reg_info.reg_length);
...
static ssize_t proc_report_point_first_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->get_report_point_first) {
		TPD_DEBUG("not support ts_ops->get_report_point_first callback\n");
		return 0;
	}
...
static ssize_t proc_report_point_first_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->set_report_point_first) {
		TPD_DEBUG("not support ts_ops->set_report_point_first callback\n");
		return count;
	}
...
static ssize_t proc_rate_white_list_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->rate_white_list_ctrl) {
		TPD_DEBUG("%s:not support ts_ops->rate_white_list_ctrl callback\n", __func__);
		return count;
	}
	...
	TPD_DEBUG("%s: write value=%d\n", __func__, value);
	if (!ts->is_suspended) {

		ts->ts_ops->rate_white_list_ctrl(ts->chip_data, value);

	} else {
		TPD_DEBUG("%s: TP is_suspended.\n", __func__);
	}
	mutex_unlock(&ts->mutex);
...
static ssize_t proc_smooth_level_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->smooth_lv_set) {
		TPD_DEBUG("%s:not support ts_ops->smooth_lv_set callback\n", __func__);
		return count;
	}
...
static ssize_t proc_sensitive_level_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (!ts->ts_ops->sensitive_lv_set) {
		TPD_DEBUG("%s:not support ts_ops->sensitive_lv_set callback\n", __func__);
		return count;
	}
...
static int calibrate_fops_read_func(struct seq_file *s, void *v)
{
	...
	if (!ts->ts_ops->calibrate) {
		TPD_DEBUG("[TP]no calibration, need back virtual calibration result");
...
static int cal_status_read_func(struct seq_file *s, void *v)
{
	...
	if (!ts->ts_ops->get_cal_status) {
		TPD_DEBUG("[TP]no calibration status, need back virtual calibration status");
			      ...
			      loff_t *ppos)
{
	...
	TPD_DEBUG("apk read is %c, count is %d.\n", (char)ts->type_now, (int)count);
	...
	if (copy_to_user(user_buf, buf, len)) {
		TPD_DEBUG("%s: can not copy the buf.\n", __func__);
		ret = -EFAULT;
		goto read_exit;
	}
	ret = len;
			       ...
			       loff_t *ppos)
{
	...
	TPD_DEBUG("apk write type is %c, count is %d.\n", (char)type,
		 (int)count);
	if (count > 1) {
		switch (type) {
		case APK_CHARGER:
...
static int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	if (ts->fw_update_app_support) {
		...
		TPD_DEBUG("register_devinfo not defined!\n");
	...
	if (prEntry_tp == NULL) {
		ret = -ENOMEM;
		TPD_DEBUG("%s: Couldn't create TP proc entry\n", __func__);
	}
	...
	if (prEntry_tmp == NULL) {
		ret = -ENOMEM;
		TPD_DEBUG("%s: Couldn't creat proc entry, %d\n", __func__, __LINE__);
	}
...
static ssize_t proc_freq_hop_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	TPD_DEBUG("%s: freq_hop_simulate value=0x%x\n", __func__, value);
	if (!ts->is_suspended) {
		mutex_lock(&ts->mutex);
		tp_freq_hop_simulate(ts, value);
		mutex_unlock(&ts->mutex);
	} else {
		TPD_DEBUG("%s: freq_hop_simulate is_suspended.\n", __func__);
		ts->freq_hop_info.freq_hop_freq = value;
	}
...
static ssize_t proc_earsense_rawdata_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (count != read_len) {
		TPD_DEBUG("%s, length:%d not match data_len:%d\n", __func__, (int)count, read_len);
		return 0;
	}
	...
	if (ts->delta_state == TYPE_DELTA_IDLE) {
		...
		if (ret) {
			TPD_DEBUG("touch rawdata read fail\n");
		}
		kfree(tmp_data);
		*ppos += 11;
	} else {
...
static ssize_t proc_earsense_delta_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (ret) {
		TPD_DEBUG("tp rawdata read fail\n");
	}
	*ppos += read_len;
	return read_len;
}
...
static ssize_t proc_earsense_selfdata_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (ts->delta_state == TYPE_DELTA_IDLE) {
		...
		if (ret) {
			TPD_DEBUG("tp self delta read fail\n");
		}
		kfree(tmp_data);
		*ppos += 11;
	} else {
...
static int tp_gesture_rate_read_func(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < sizeof(coord_arg) / sizeof(coord_arg[0]); i++) {
		...
		TPD_DEBUG("test row %d done\n", i);
	}
	TPD_DEBUG("ALL Test %d Group Data, fail:%d", i, fail_count);
...
static int init_debug_info_proc(struct touchpanel_data *ts)
{
	...
	if (prEntry_debug_info == NULL) {
		ret = -ENOMEM;
		TPD_DEBUG("%s: Couldn't create debug_info proc entry\n", __func__);
	}
	...
	if (ts->smooth_level_array_support || ts->smooth_level_charging_array_support ||
		...
		if (prEntry_tmp == NULL) {
			ret = -ENOMEM;
			TPD_DEBUG("%s: Couldn't create para_support, %d\n", __func__, __LINE__);
		}
	}
...
static int init_input_device(struct touchpanel_data *ts)
{
	...
	if (ts->input_dev == NULL) {
		ret = -ENOMEM;
		TPD_DEBUG("Failed to allocate input device\n");
		return ret;
	}
	...
	if (ts->kpd_input_dev == NULL) {
		ret = -ENOMEM;
		TPD_DEBUG("Failed to allocate key input device\n");
		return ret;
	}
	...
	if (ts->face_detect_support) {
		...
		if (ts->ps_input_dev == NULL) {
			ret = -ENOMEM;
			TPD_DEBUG("Failed to allocate ps input device\n");
			return ret;
		}
	...
	if (ts->pen_support) {
		...
		if (ts->pen_input_dev == NULL) {
			ret = -ENOMEM;
			TPD_DEBUG("Failed to allocate pen input device\n");
			return ret;
		}
	...
	if (input_register_device(ts->input_dev)) {
		TPD_DEBUG("%s: Failed to register input device\n", __func__);
		input_free_device(ts->input_dev);
		return -1;
	}

	if (input_register_device(ts->kpd_input_dev)) {
		TPD_DEBUG("%s: Failed to register key input device\n", __func__);
	...
	if (ts->face_detect_support) {
		if (input_register_device(ts->ps_input_dev)) {
			TPD_DEBUG("%s: Failed to register ps input device\n", __func__);
	...
	if (ts->pen_support) {
		...
		if (input_register_device(ts->pen_input_dev)) {
			TPD_DEBUG("%s: Failed to register pen input device\n", __func__);
...
static int init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
	...
	if (ts->iovcc_shutdown_support) {
		...
		if (gpio_is_valid(ts->hw_res.cs_gpio)) {
			...
			if (rc) {
				TPD_DEBUG("nvt unable to request gpio [%d]\n", ts->hw_res.cs_gpio);
			} else {
				TPD_DEBUG("nvt success to request gpio [%d]\n", ts->hw_res.cs_gpio);
			}
		} else {
			TPD_DEBUG("nvt ts->cs-gpio not specified\n");
		}
	}
	...
	if (!rc) {
		TPD_DEBUG("project not specified\n");
	}
	if (ts->panel_data.project_num > 0) {
		...
		if (rc) {
			TPD_DEBUG("platform_support_project not specified");
			return -1;
		}
		...
		if (rc) {
			TPD_DEBUG("platform_support_project_dir not specified");
			return -1;
		}
		for (i = 0; i < ts->panel_data.project_num; i++) {
			...
			if (ts->panel_data.platform_support_commandline[i] == NULL || ts->panel_data.platform_support_external_name[i] == NULL) {
				TPD_DEBUG("panel_data.platform_support_commandline or platform_support_external_name kzalloc error\n");
			...
			if (rc) {
				TPD_DEBUG("platform_support_project_commandline not specified");
				goto commandline_kazalloc_error;
			}
			...
			if (rc) {
				TPD_DEBUG("platform_support_project_external_name not specified");
			}
		}
		if (!tp_judge_ic_match_commandline(&ts->panel_data)) {
			TPD_DEBUG("commandline not match, please update dts");
			goto commandline_kazalloc_error;
		}
	} else {
		TPD_DEBUG("project and commandline not specified in dts, please update dts");
	}
	rc = of_property_read_u32(np, "vid_len", &ts->panel_data.vid_len);
	if (rc) {
		TPD_DEBUG("panel_data.vid_len not specified\n");
	}
	rc = of_property_read_u32(np, "tp_type", &ts->panel_data.tp_type);
	if (rc) {
		TPD_DEBUG("tp_type not specified\n");
	}
	rc = of_property_read_u32(np, "report_rate_limit", &ts->panel_data.report_rate_limit);
	if (rc) {
		TPD_DEBUG("report_rate_limit not specified\n");
	...
	if (!ts->sportify_aod_gesture_support) {
		TPD_DEBUG("not support sportify_aod_gesture\n");
	}
	rc = of_property_read_u32(np, "smooth_level", &val);
	if (rc) {
		TPD_DEBUG("smooth_level not specified\n");
	} else {
	...
	if (rc) {
		TPD_DEBUG("oplus_smooth_level not specified %d\n", rc);
	}
	TPD_DEBUG("ts->oplus_smooth_level= %d\n", ts->oplus_smooth_level);

	rc = of_property_read_u32(np, "vdd_2v8_volt", &ts->hw_res.vdd_volt);
	if (rc < 0) {
		ts->hw_res.vdd_volt = 0;
		TPD_DEBUG("vdd_2v8_volt not defined\n");
	}
	...
	if (rc < 0) {
		TPD_DEBUG("failed to get chip name, firmware/limit name will be invalid\n");
	}
	if (gpio_is_valid(ts->hw_res.irq_gpio)) {
		...
		TPD_DEBUG("irq-gpio not specified in dts\n");
	}
	...
	TPD_DEBUG("%s : irq_gpio = %d, irq_flags = 0x%x, reset_gpio = %d\n",
	...
	if (gpio_is_valid(ts->hw_res.cs_gpio)) {
		...
		TPD_DEBUG("ts->cs-gpio not specified\n");
	}
	...
	if (gpio_is_valid(ts->hw_res.id1_gpio)) {
		...
		TPD_DEBUG("id1_gpio not specified\n");
	}
	...
	if (gpio_is_valid(ts->hw_res.id2_gpio)) {
		...
		TPD_DEBUG("id2_gpio not specified\n");
	}
	...
	if (ts->hw_res.enable1v8_gpio < 0) {
		TPD_DEBUG("ts->hw_res.enable1v8_gpio not specified\n");
	} else {
	...
	if (rc) {
		TPD_DEBUG("int-mode not specified\n");
	} else {
	...
	if (rc) {
		TPD_DEBUG("No firmware in dts !\n");
	} else {
		TPD_DEBUG("The firmware len id %d!\n", val);
		...
		if (ts->firmware_in_dts != NULL) {
			...
			if (ts->firmware_in_dts->data == NULL) {
				...
				if (rc) {
					TPD_DEBUG("Can not get the firmware in dts!\n");
	...
	if (rc) {
		TPD_DEBUG("ts->irq_need_dev_resume_time not specified\n");
		ts->irq_need_dev_resume_time = 50;
	}
	TPD_DEBUG("ts->irq_need_dev_resume_time = %d \n", ts->irq_need_dev_resume_time);
	// resolution info
	...
	if (rc) {
		TPD_DEBUG("ts->msecs_to_jiffies_time not specified\n");
	...
	TPD_DEBUG("LCD_WIDTH = %d, LCD_HEIGHT = %d, max_x = %d, max_y = %d, limit_witdh = %d, limit_height = %d\n",
	...
	if (ts->health_monitor_support) {
		...
		TPD_DEBUG("eli_size = %d, eli_ver_range = %d, eli_hor_range = %d\n",
	...
	if (rc) {
		TPD_DEBUG("smooth_level_array not specified %d\n", rc);
	} else {
	...
	if (rc) {
		TPD_DEBUG("smooth_level_charging_array not specified %d\n", rc);
	} else {
	...
	if (rc) {
		TPD_DEBUG("sensitive_level_array not specified %d\n", rc);
	} else {
	...
	if (rc) {
		TPD_DEBUG("sensitive_level_charging_array not specified %d\n", rc);
	} else {
	...
	if (rc < 0) {
		TPD_DEBUG("error:game_perf_para_default should be setting in dts!\n");
		...
		TPD_DEBUG("sensitive_level_default is %d, smooth_level_default is %d\n",
	...
	if (rc) {
		TPD_DEBUG("stop_filter_set not specified %d\n", rc);
	} else {
		TPD_DEBUG("stop_filter_set is %d\n", temp_array[0]);
	...
	if (rc) {
		...
		TPD_DEBUG("set default horizontal area value:%d.\n", ts->default_hor_area);
	}

	// virturl key Related
	rc = of_property_read_u32_array(np, "touchpanel,button-type", temp_array, 2);
	if (rc < 0) {
		TPD_DEBUG("error:button-type should be setting in dts!");
	} else {
...
int init_power_control(struct touchpanel_data *ts)
{
	...
	if (IS_ERR_OR_NULL(ts->hw_res.vcc_1v8)) {
		TPD_DEBUG("Regulator get failed vcc_1v8, ret = %d\n", ret);
	} else {
	...
	if (IS_ERR_OR_NULL(ts->hw_res.vdd_2v8)) {
		TPD_DEBUG("Regulator vdd2v8 get failed, ret = %d\n", ret);
	} else {
		if (regulator_count_voltages(ts->hw_res.vdd_2v8) > 0) {
			TPD_DEBUG("set avdd voltage to %d uV\n", ts->hw_res.vdd_volt);
...
int tp_powercontrol_1v8(struct hw_resource *hw_res, bool on)
{
	...
	if (on) { // 1v8 power on
		if (!IS_ERR_OR_NULL(hw_res->vcc_1v8)) {
			TPD_DEBUG("Enable the Regulator1v8.\n");
			ret = regulator_enable(hw_res->vcc_1v8);
			if (ret) {
				TPD_DEBUG("Regulator vcc_i2c enable failed ret = %d\n", ret);
				return ret;
			}
		}

		if (hw_res->enable1v8_gpio > 0) {
			TPD_DEBUG("Enable the 1v8_gpio\n");
			ret = gpio_direction_output(hw_res->enable1v8_gpio, 1);
			if (ret) {
				TPD_DEBUG("enable the enable1v8_gpio failed.\n");
				return ret;
			}
		}
	} else { // 1v8 power off
		if (!IS_ERR_OR_NULL(hw_res->vcc_1v8)) {
			...
			if (ret) {
				TPD_DEBUG("Regulator vcc_i2c enable failed rc = %d\n", ret);
				return ret;
			}
		}

		if (hw_res->enable1v8_gpio > 0) {
			TPD_DEBUG("disable the 1v8_gpio\n");
			ret = gpio_direction_output(hw_res->enable1v8_gpio, 0);
			if (ret) {
				TPD_DEBUG("disable the enable2v8_gpio failed.\n");
				return ret;
			}
		}
	}
...
int tp_powercontrol_2v8(struct hw_resource *hw_res, bool on)
{
	...
	if (on) { // 2v8 power on
		if (!IS_ERR_OR_NULL(hw_res->vdd_2v8)) {
			TPD_DEBUG("Enable the Regulator2v8.\n");
			ret = regulator_enable(hw_res->vdd_2v8);
			if (ret) {
				TPD_DEBUG("Regulator vdd enable failed ret = %d\n", ret);
				return ret;
			}
		}
		if (hw_res->enable2v8_gpio > 0) {
			TPD_DEBUG("Enable the 2v8_gpio, hw_res->enable2v8_gpio is %d\n", hw_res->enable2v8_gpio);
			ret = gpio_direction_output(hw_res->enable2v8_gpio, 1);
			if (ret) {
				TPD_DEBUG("enable the enable2v8_gpio failed.\n");
				return ret;
			}
		}
	} else { // 2v8 power off
		if (!IS_ERR_OR_NULL(hw_res->vdd_2v8)) {
			...
			if (ret) {
				TPD_DEBUG("Regulator vdd disable failed rc = %d\n", ret);
				return ret;
			}
		}
		if (hw_res->enable2v8_gpio > 0) {
			TPD_DEBUG("disable the 2v8_gpio\n");
...
static void esd_handle_func(struct work_struct *work)
{
	...
	if (ts->loading_fw) {
		TPD_DEBUG("FW is updating, stop esd handle!\n");
		return;
	}

	mutex_lock(&ts->esd_info.esd_lock);
	if (!ts->esd_info.esd_running_flag) {
		TPD_DEBUG("Esd protector has stopped!\n");
		goto ESD_END;
	}

	if (ts->is_suspended == 1) {
		TPD_DEBUG("Touch panel has suspended!\n");
		goto ESD_END;
	}

	if (!ts->ts_ops->esd_handle) {
		TPD_DEBUG("not support ts_ops->esd_handle callback\n");
		goto ESD_END;
	}
	...
	if (ts->esd_info.esd_running_flag) {
		...
		TPD_DEBUG("Esd protector suspended!");
	}
...
void esd_handle_switch(struct esd_information *esd_info, bool on)
{
	...
	if (on) {
		if (!esd_info->esd_running_flag) {
			...
			TPD_DEBUG("Esd protector started, cycle: %d s\n", esd_info->esd_work_time / HZ);
		...
		if (esd_info->esd_running_flag) {
			...
			TPD_DEBUG("Esd protector stoped!\n");
...
int tp_register_irq_func(struct touchpanel_data *ts)
{
	...
	if (gpio_is_valid(ts->hw_res.irq_gpio)) {
		...
		if (ts->irq_flags_cover) {
			...
			TPD_DEBUG("%s irq_flags is covered by 0x%x\n", __func__, ts->irq_flags_cover);
		}
		...
		ret = request_threaded_irq(ts->irq, NULL,
					   ...
					   TPD_DEVICE, ts);
		if (ret < 0) {
			TPD_DEBUG("%s request_threaded_irq ret is %d\n", __func__, ret);
		}
	} else {
		TPD_DEBUG("%s:no valid irq\n", __func__);
	}
#else
...
static void tp_async_secdebug(void *data)
{
	...
	if (kobject_uevent_env(&ts->dev->kobj, KOBJ_CHANGE, env)) {
		TPD_DEBUG("failed to send uevent");
	}
...
int register_common_touch_device(struct touchpanel_data *pdata)
{
	...
	if (ts->health_monitor_v2_support) {
		...
		if (ret < 0) {
			TPD_DEBUG("health info init failed.\n");
		}
	...
	if (ret) {
		TPD_DEBUG("%s: tp power init failed.\n", __func__);
		return -1;
	}
	ret = reconfig_power_control(ts);
	if (ret) {
		TPD_DEBUG("%s: reconfig power failed.\n", __func__);
		return -1;
	}
	if (!ts->ts_ops->power_control) {
		ret = -EINVAL;
		TPD_DEBUG("tp power_control NULL!\n");
	...
	if (ret < 0) {
		ret = -EINVAL;
		TPD_DEBUG("tp_input_init failed!\n");
		goto err_check_functionality_failed;
	}
...
#endif
	{
		...
		g_tp = ts;
		TPD_DEBUG("%s: not int normal mode, return.\n", __func__);
		return ret;
	}
	...
	if (!ts->ts_ops->get_chip_info) {
		ret = -EINVAL;
		TPD_DEBUG("tp get_chip_info NULL!\n");
		goto err_check_functionality_failed;
	}
	ret = ts->ts_ops->get_chip_info(ts->chip_data);
	if (ret < 0) {
		ret = -EINVAL;
		TPD_DEBUG("tp get_chip_info failed!\n");
		goto err_check_functionality_failed;
	}
	...
	if (!ts->is_noflash_ic) {           //noflash don't have firmware before fw update
		if (!ts->ts_ops->fw_check) {
			ret = -EINVAL;
			TPD_DEBUG("tp fw_check NULL!\n");
			goto manu_info_alloc_err;
		}
		ret = ts->ts_ops->fw_check(ts->chip_data, &ts->resolution_info, &ts->panel_data);
		if (ret == FW_ABNORMAL) {
			ts->force_update = 1;
			TPD_DEBUG("This FW need to be updated!\n");
		} else {
			ts->force_update = 0;
		}
	}
	...
	if (!ts->ts_ops->mode_switch) {
		ret = -EINVAL;
		TPD_DEBUG("tp mode_switch NULL!\n");
		goto manu_info_alloc_err;
	}
	ret = ts->ts_ops->mode_switch(ts->chip_data, MODE_NORMAL, true);
	if (ret < 0) {
		ret = -EINVAL;
		TPD_DEBUG("%s:modem switch failed!\n", __func__);
		goto manu_info_alloc_err;
	}
	...
	if (ts->spurious_fp_support) {
		...
		if (IS_ERR(ts->spuri_fp_touch.thread)) {
			TPD_DEBUG("spurious fingerprint thread create failed\n");
		}
	}
	...
	if (ts->ear_sense_support) {
		...
		if (ts->earsense_delta == NULL) {
			ret = -ENOMEM;
			TPD_DEBUG("earsense_delta kzalloc error\n");
			goto threaded_irq_free;
		}
	...
	if (ts->health_monitor_support) {
		...
		if ((NULL == ts->monitor_data.eli_ver_pos) || (NULL == ts->monitor_data.eli_hor_pos)) {
			TPD_DEBUG("array for elimination area kzalloc failed.\n");
		}
	}
	...
	TPD_DEBUG("Touch panel probe : normal end\n");
	return 0;
...
static int tp_suspend(struct device *dev)
{
	...
	TPD_DEBUG("tp_suspend ts->spuri_fp_touch.fp_trigger =%d  ts->i2c_ready =%d  ts->spuri_fp_touch.lcd_resume_ok=%d \n",
	...
	if (ts->input_dev == NULL) {
		TPD_DEBUG("input_dev  registration is not complete\n");
		goto NO_NEED_SUSPEND;
	}
	if (ts->loading_fw) {
		TPD_DEBUG("FW is updating while suspending");
		goto NO_NEED_SUSPEND;
	}
	...
	if (!ts->is_suspended) {
		...
		TPD_DEBUG("%s: do not suspend twice.\n", __func__);
		goto EXIT;
	}
...
EXIT:
	...
	if ((ts->iovcc_shutdown_support) && (!ts->gesture_enable)) {
		TPD_DEBUG("%s: try to control cs status in suspend status.\n", __func__);
		if (gpio_get_value(ts->hw_res.cs_gpio)) {
			TPD_DEBUG("%s: nvt try to set cs status output low in suspend status.\n", __func__);
			ret = gpio_direction_output(ts->hw_res.cs_gpio, 0);
			if (ret < 0) {
				TPD_DEBUG("nvt %s: failed control cs status end.\n", __func__);
			}
		}
	}
...
static void tp_resume(struct device *dev)
{
	...
	if (!ts->is_suspended) {
		TPD_DEBUG("%s: do not resume twice.\n", __func__);
		goto NO_NEED_RESUME;
	}
	ts->monitor_data.monitor_down = 0;
	ts->monitor_data.monitor_up = 0;
...
static void lcd_trigger_load_tp_fw(struct work_struct *work)
{
	...
	if (ts->lcd_trigger_load_tp_fw_support) {
		if (is_running) {
			TPD_DEBUG("%s is running, can not repeat\n", __func__);
		} else {
			TPD_DEBUG("%s start\n", __func__);
			if (ts->health_monitor_v2_support) {
				reset_healthinfo_time_counter(&start_time);
...
static void speedup_resume(struct work_struct *work)
{
	...
	if (ts->use_resume_notify && (!ts->fp_info.touch_state || !ts->report_flow_unlock_support)) {
		...
		if ((0 == timed_out) || (ts->resume_complete.done)) {
			TPD_DEBUG("resume state, timed_out:%d, done:%d\n", timed_out, ts->resume_complete.done);
		...
		if (*blank == FB_BLANK_POWERDOWN) { //suspend
			if (event == FB_EARLY_EVENT_BLANK) {    //early event
				...
				if ((0 == timed_out) || (ts->pm_complete.done)) {
					TPD_DEBUG("completion state, timed_out:%d, done:%d\n", timed_out, ts->pm_complete.done);
				}
...
{
	if (g_tp) {
		TPD_DEBUG("%s:common panel struct has alloc already!\n", __func__);
		return NULL;
	}
