...
static void oplus_chg_wls_status_keep_clean_work(struct work_struct *work)
{
	...
	if (chip == NULL || info == NULL) {
		...
		if (chip->wls_status_keep == WLS_SK_BY_HAL) {
			...
			queue_delayed_work(system_power_efficient_wq, &info->status_keep_clean_work, msecs_to_jiffies(5000));
			return;
		}
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (chip->wls_status_keep != WLS_SK_NULL) {
			...
			if (pre_wls_online && pval->intval == 0) {
				...
				pval->intval = 1;
				queue_delayed_work(system_power_efficient_wq, &info->status_keep_clean_work, msecs_to_jiffies(KEEP_CLEAN_INTERVAL));
			} else {
				pre_wls_online = pval->intval;
				if (info->status_wake_lock_on) {
					cancel_delayed_work_sync(&info->status_keep_clean_work);
					queue_delayed_work(system_power_efficient_wq, &info->status_keep_clean_work, 0);
				}
			}
		}
		break;
