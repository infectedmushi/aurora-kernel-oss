...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (g_oplus_chip->hiz_gnd_cable && !vbus_rising) {
		...
		if (!ccdetect_level && fastchging) {
			...
			queue_delayed_work(system_power_efficient_wq, &chg->connect_check_work, msecs_to_jiffies(OPLUS_CONNECT_CHK_INTERVAL_MS));
		} else if (!fastchging) {
...
static void register_oplus_pdsvooc_svid(struct work_struct *work) {
	...
	if (IS_ERR(pd)) {
			...
			rc = PTR_ERR(pd);
			chg->oplus_pd = NULL;
			queue_delayed_work(system_power_efficient_wq, &chg->regist_pd, msecs_to_jiffies(1000));
		} else {
				chg_err("oplus pps usbpd phandle failed (%ld)\n", PTR_ERR(pd));
				chg->oplus_pd = pd;
