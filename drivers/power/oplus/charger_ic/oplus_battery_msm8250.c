...
irqreturn_t oplus_wired_conn_change_handler(int irq, void *data)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chg->wired_in_work, 0);
...
void oplus_switch_to_wired_charge(struct smb_charger *chg)
{
	...
	if (oplus_wpc_get_otg_charging()) {
		...
		queue_delayed_work(system_power_efficient_wq, &chg->wait_wired_charge_on, msecs_to_jiffies(100));
	}
}
...
void oplus_switch_from_wired_charge(struct smb_charger *chg)
{
	if (oplus_wpc_get_otg_charging()) {
		...
		queue_delayed_work(system_power_efficient_wq, &chg->wait_wired_charge_off, msecs_to_jiffies(100));
	} else {
		oplus_wpc_set_vbat_en_val(0);
...
int smblib_vbus_regulator_disable(struct regulator_dev *rdev)
{
	...
	if (chip->vbatt_num == 2) {
		...
		if (chip->wireless_support) {
			...
			queue_delayed_work(system_power_efficient_wq, &chg->otg_disable_timeout_work, msecs_to_jiffies(100));
				    ...
				    const union power_supply_propval *val)
{
	...
	queue_delayed_work(system_power_efficient_wq, &chg->reset_rd_work, msecs_to_jiffies(500));
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	if (g_oplus_chip && g_oplus_chip->wireless_support) {
		...
		queue_delayed_work(system_power_efficient_wq, &chg->switch_to_wired_work, 0);
	}
#endif
	...
	if (vbus_rising) {
		...
		if (!(chg->hvdcp_detect_ok && (chg->hvdcp_detach_time - chg->hvdcp_detect_time <= OPLUS_HVDCP_DETECT_TO_DETACH_TIME))) {
			...
			queue_delayed_work(system_power_efficient_wq, &chg->hvdcp_disable_work, OPLUS_HVDCP_DISABLE_INTERVAL);
		}
#endif
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		queue_delayed_work(system_power_efficient_wq, &chg->typec_disable_cmd_work, msecs_to_jiffies(500));
#endif
	...
	if (g_oplus_chip->hiz_gnd_cable && !vbus_rising) {
		...
		if (!ccdetect_level && fastchging) {
			...
			queue_delayed_work(system_power_efficient_wq, &chg->connect_check_work, msecs_to_jiffies(1000));
		} else if (!fastchging) {
...
static void oplus_ccdetect_work(struct work_struct *work)
{
	...
	if (level != 1) {
		...
		smblib_set_usb_suspend(chg, true);
		queue_delayed_work(system_power_efficient_wq, &chg->recovery_suspend_work, REC_SUSPEND_WORK_SCH_DELAY);
#endif
	}
#ifdef OPLUS_CUSTOM_OP_DEF
...
static int smb5_determine_initial_status(struct smb5 *chip)
{
    ...
    if (chg->early_usb_attach)
        queue_delayed_work(system_power_efficient_wq, &chg->typec_disable_cmd_work, msecs_to_jiffies(1000));
#endif
...
static void register_oplus_pdsvooc_svid(struct work_struct *work) {
	...
	if (IS_ERR(pd)) {
		...
		queue_delayed_work(system_power_efficient_wq, &chg->regist_pd, msecs_to_jiffies(PD_SVOOC_SVID_MS));
	} else {
		//msleep(1500);
...
//kong
	...
	queue_delayed_work(system_power_efficient_wq, &chg->regist_pd, 0);
