...
static void pxa_cpufreq_init_voltages(void)
{
	...
	if (IS_ERR(vcc_core)) {
		pr_debug("Didn't find vcc_core regulator\n");
		vcc_core = NULL;
	} else {
		pr_debug("Found vcc_core regulator\n");
	}
}
#else
static int pxa_cpufreq_change_voltage(const struct pxa_freqs *pxa_freq)
...
static void pxa27x_guess_max_freq(void)
{
	if (!pxa27x_maxfreq) {
		...
		pr_debug("PXA CPU 27x max frequency not defined (pxa27x_maxfreq), assuming pxa271 with %dkHz maxfreq\n",
			pxa27x_maxfreq);
	} else {
		pxa27x_maxfreq *= 1000;
	}
...
static int pxa_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (cpu_is_pxa25x()) {
		...
		pr_debug("using %s frequency table\n",
	...
	pr_debug("frequency change support initialized\n");
