...
static void pcwd_show_card_info(void)
{
	...
	if (pcwd_private.revision == PCWD_REVISION_A)
		pr_debug("ISA-PC Watchdog (REV.A) detected at port 0x%04x\n",
			pcwd_private.io_addr);
	else if (pcwd_private.revision == PCWD_REVISION_C) {
		pcwd_get_firmware();
		pr_debug("ISA-PC Watchdog (REV.C) detected at port 0x%04x (Firmware version: %s)\n",
	...
	if (pcwd_private.supports_temp)
		pr_debug("Temperature Option Detected\n");

	if (pcwd_private.boot_status & WDIOF_CARDRESET)
		pr_debug("Previous reboot was caused by the card\n");
...
static int pcwd_start(void)
{
	...
	if (pcwd_private.revision == PCWD_REVISION_C) {
		...
		spin_unlock(&pcwd_private.io_lock);
		if (stat_reg & WD_WDIS) {
			pr_debug("Could not start watchdog\n");
			return -EIO;
		}
	}
...
static int pcwd_stop(void)
{
	...
	if (pcwd_private.revision == PCWD_REVISION_C) {
		...
		if ((stat_reg & WD_WDIS) == 0) {
			pr_debug("Could not stop watchdog\n");
			return -EIO;
		}
	}
	...
	if (pcwd_private.revision == PCWD_REVISION_A) {
		...
		if (control_status & WD_T110) {
			*status |= WDIOF_OVERHEAT;
			if (temp_panic) {
				pr_debug("Temperature overheat trip!\n");
				kernel_power_off();
			}
		}
	} else {
...
static int pcwd_clear_status(void)
{
	...
	if (pcwd_private.revision == PCWD_REVISION_C) {
		...
		if (debug >= VERBOSE)
			pr_debug("clearing watchdog trip status\n");
...
static int pcwd_isa_match(struct device *dev, unsigned int id)
{
	...
	if (!request_region(base_addr, 4, "PCWD")) {
		pr_debug("Port 0x%04x unavailable\n", base_addr);
		return 0;
	}
...
static int pcwd_isa_probe(struct device *dev, unsigned int id)
{
	...
	cards_found++;
	if (cards_found == 1)
		pr_debug("v%s Ken Hollis (kenji@bitgate.com)\n",
							WATCHDOG_VERSION);
	...
	if (pcwd_set_heartbeat(heartbeat)) {
		...
		pr_debug("heartbeat value must be 2 <= heartbeat <= 7200, using %d\n",
			WATCHDOG_HEARTBEAT);
	}
